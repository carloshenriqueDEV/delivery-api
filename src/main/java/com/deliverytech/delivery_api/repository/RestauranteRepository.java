...
import com.deliverytech.delivery_api.service.dtos.RelatorioTotalVendasPorRestauranteDTO;
...
public interface RestauranteRepository extends JpaRepository<Restaurante, Long> { 
    ...
    // Buscar por taxa de entrega menor ou igual a um valor específico
    List<Restaurante> findByTaxaEntregaLessThanEqual(BigDecimal taxa);

    // Relatório - total de vendas por restaurante
    @Query("SELECT r.nome, SUM(p.valorTotal) FROM Restaurante r JOIN r.pedidos p " + 
           "WHERE p.status = 'ENTREGUE' GROUP BY r.id ORDER BY SUM(p.valorTotal) DESC")
    List<RelatorioTotalVendasPorRestauranteDTO> calcularTotalVendasPorRestaurante(); 
} 